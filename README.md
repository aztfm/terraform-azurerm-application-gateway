...
|autoscale\_configuration|n/a|`map(string)`|`{}`|no|
...
|capacity|The Capacity to use for this Application Gateway. When using a v1 SKU this value must be between `1` and `32`, and `1` to `125` for a v2 SKU. `Standard_Small`, `Standard_Medium`, `Standard_Large`, `Standard_v2`, `WAF_Medium`, `WAF_Large` and `WAF_v2`.|`string`|`null`|no|
...
|private\_ip\_address|A list of IP Addresses which should be part of the Backend Address Pool.|`string`|`null`|no|
